function __print_palladium_functions_help() {
cat <<EOF
Additional Palladium-OS functions:
- cout:            Changes directory to out.
- mmp:             Builds all of the modules in the current directory and pushes them to the device.
- mmap:            Builds all of the modules in the current directory and its dependencies, then pushes the package to the device.
- mmmp:            Builds all of the modules in the supplied directories and pushes them to the device.
- palladiumgerrit:   A Git wrapper that fetches/pushes patch from/to PalladiumOS Gerrit Review.
- palladiumrebase:   Rebase a Gerrit change and push it again.
- palladiumremote:   Add git remote for PalladiumOS Gerrit Review.
- aospremote:      Add git remote for matching AOSP repository.
- cafremote:       Add git remote for matching CodeAurora repository.
- githubremote:    Add git remote for Palladium-OS Github.
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the palladium model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi

            lunch palladium_$target-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/palladium-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.palladium.device | grep -q "$PALLADIUM_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $PALLADIUM_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function palladiumremote()
{
    ...
    git remote rm palladium 2> /dev/null
    local REMOTE=$(git config --get remote.github.projectname)
    local PALLADIUM="true"
    if [ -z "$REMOTE" ]
    then
    ...
    then
        ...
        local PFX="PalladiumOS/"
    else
        local PROJECT=$REMOTE
    fi

    local PALLADIUM_USER=$(git config --get review.review.palladiumos.org.username)
    if [ -z "$PALLADIUM_USER" ]
    then
        git remote add palladium ssh://review.palladiumos.org:29418/$PFX$PROJECT
    else
        git remote add palladium ssh://$PALLADIUM_USER@review.palladiumos.org:29418/$PFX$PROJECT
    fi
    echo "Remote 'palladium' created"
}
...
function githubremote()
{
    ...
    git remote add github https://github.com/Palladium-OS/$PROJECT
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.palladium.device | grep -q "$PALLADIUM_BUILD");
    then
...
function makerecipe() {
    ...
    then
        pwd
        palladiumremote
        git push palladium HEAD:refs/heads/'$1'
    fi
    '
}
...
function palladiumgerrit() {
    ...
    local user=`git config --get review.review.palladiumos.org.username`
...
EOF
            ...
            case $1 in
                ...
                changes|for)
                    if [ "$FUNCNAME" = "palladiumgerrit" ]; then
...
function palladiumrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "Palladium-OS Gerrit Rebase Usage: "
        echo "      palladiumrebase <path to project> <patch IDs on Gerrit>"
    ...
    git fetch "http://review.palladiumos.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function dopush()
{
    ...
    if (adb shell getprop ro.palladium.device | grep -q "$PALLADIUM_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function repopick() {
    T=$(gettop)
    $T/vendor/palladium/build/tools/repopick.py $@
}
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $PALLADIUM_FIXUP_COMMON_OUT ]; then
